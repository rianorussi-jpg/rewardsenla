...
- `google-create-class`: crea/actualiza una LoyaltyClass por negocio y guarda `google_class_id` en `loyalty_programs`.
- `google-create-object`: crea un LoyaltyObject por cliente y guarda `google_object_id` en `customers`; devuelve el enlace/JWT de **Add to Google Wallet**.
...
El panel ya guarda `businesses.slug`. La siguiente pieza de producción es una ruta pública como `/r/mi-negocio` donde:
1. El cliente escribe nombre/teléfono/email.
2. Se crea un registro en `customers` mediante una Edge Function pública con validación/rate limit (NO abras INSERT anónimo directo a la tabla).
...
- Registra cada cambio de puntos/sellos en `loyalty_transactions`.
- Para evitar fraude, idealmente el incremento de sellos se hace vía una función RPC/Edge Function que valide que el usuario pertenece al negocio, en vez de actualizar `customers.current_value` directamente desde el navegador.
