...
async function startCheckout(plan,publishProgramId=null,billingInterval='monthly'){ensureConfigured();const {data,error}=await sb.functions.invoke('create-checkout-session',{body:{plan,publishProgramId,billingInterval}});if(error)throw error;if(data?.error)throw new Error(data.error);if(data?.updated)return data;if(!data?.url)throw new Error('Stripe no devolvió la página de pago.');location.href=data.url;return data}
async function getAnnualPromoStatus(){ensureConfigured();const {data,error}=await sb.rpc('rewards_annual_promo_status');if(error)throw error;return Array.isArray(data)?data[0]:data}
