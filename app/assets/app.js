...
async function getBusiness(){if(!sb){const d=demoStore();return d.business||{id:'demo-business',name:'Mi negocio'}}const u=await currentUser();if(!u)return null;const {data,error}=await sb.from('businesses').select('*').eq('owner_id',u.id).maybeSingle();if(error)throw error;return data}
async function getProgram(){if(!sb){const d=demoStore();return d.program||null}const b=await getBusiness();if(!b)return null;const {data,error}=await sb.from('loyalty_programs').select('*').eq('business_id',b.id).maybeSingle();if(error)throw error;return data}
async function saveProgram(p){if(!sb){const d=demoStore();d.program={...(d.program||{}),...p,id:d.program?.id||'demo-program'};saveDemo(d);return d.program}const b=await getBusiness();const payload={...p,business_id:b.id};delete payload.id;const existing=await getProgram();if(existing){const {data,error}=await sb.from('loyalty_programs').update(payload).eq('id',existing.id).select().single();if(error)throw error;return data}else{const {data,error}=await sb.from('loyalty_programs').insert(payload).select().single();if(error)throw error;return data}}
