...
Deno.serve(async req=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors});try{
 ...
 const {plan}=await req.json(); if(!['basic','pro','business'].includes(plan))throw new Error('Plan inválido.');
 ...
 const stripe=new Stripe(Deno.env.get('STRIPE_SECRET_KEY')!); let customer=b.stripe_customer_id;
 ...
 const origin=req.headers.get('origin')||Deno.env.get('APP_URL')||'https://rewards.enla.mx';
 const s=await stripe.checkout.sessions.create({mode:'subscription',customer,line_items:[{price:priceMap[plan],quantity:1}],success_url:`${origin}/app/billing.html?success=1`,cancel_url:`${origin}/app/billing.html?canceled=1`,allow_promotion_codes:true,subscription_data:{metadata:{business_id:b.id,plan}},metadata:{business_id:b.id,plan}});
